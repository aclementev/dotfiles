...
local function remove_qf_entry()
  ...
  for i, entry in ipairs(old_entries) do
    if i ~= line then table.insert(new_entries, entry) end
  end
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function()
    -- Open the file
    vim.keymap.set("n", "<C-V>", function() open_in_vertical() end, { buffer = true })
    vim.keymap.set("n", "<C-X>", function() open_in_horizontal() end, { buffer = true })
    -- TODO(alvaro): Ideally we want to make this more generic, allowing for count and other deletion
    -- actions (e.g. visual selection)
    vim.keymap.set("n", "dd", function() remove_qf_entry() end, { buffer = true })
  end,
})
